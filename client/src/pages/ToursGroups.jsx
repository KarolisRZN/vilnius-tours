...
export default function ToursGroups() {
  ...
  const fetchTours = async () => {
    setLoading(true);
    const res = await fetch("/api/tours?category=group");
  ...
  return (
    <section className="w-full bg-white py-10 px-4 flex flex-col items-center">
      <h2 className="text-3xl md:text-4xl font-bold mb-8 text-green-700 text-center">
        Group Tours In Vilnius
      </h2>
      {loading ? (
        <p className="text-gray-500 text-center">Loading...</p>
      ) : tours.length === 0 ? (
        <p className="text-gray-500 text-center">No group tours available.</p>
      ) : (
